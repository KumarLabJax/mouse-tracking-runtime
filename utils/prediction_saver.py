...
class prediction_saver:
	...
	def __init__(self, resize_increment: int = 10000, dtype: np.dtype = np.float32, pad_value: float = 0):
	...
	def dequeue_thread(self, results_queue, output_queue):
		...
		while True:
			...
			if prediction_matrix is None:
				...
				next_mat_size[1:] = np.max([cur_mat_size[1:], predictions.shape[1:]], axis=0)
				if np.any(next_mat_size != cur_mat_size):
					prediction_matrix = self.__resize_prediction_mat(prediction_matrix, next_mat_size)
				# Pad predictions for lazy slicing
				adjusted_prediction_shape = next_mat_size.copy()
				adjusted_prediction_shape[0] = prediction_count
				resized_predictions = self.__resize_prediction_mat(predictions[:prediction_count], adjusted_prediction_shape)
				# Copy in new data
	...
	def start_dequeue_results(self):
		...
		if self.__results_storage_thread is None:
			self.__results_storage_thread = mp.Process(target=self.dequeue_thread, args=(self.results_receiver_queue, self.results_queue,), daemon=True)
