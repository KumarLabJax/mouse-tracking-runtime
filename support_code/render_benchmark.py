...
def parse_trace(file: Path, delimiter: str = '\t') -> pd.DataFrame:
    ...
    return_df['peak_rss'] = return_df['rss_coeff'] * return_df['rss_unit'].map(unit_conversions)
...
scaling_df['video_duration_min'] = [int(x.groups(0)[0]) if x is not None else 0 for x in tmp]
...
df_melted = pd.melt(pd.concat([scaling_df, hr_df]), id_vars=['video_duration_min', 'process', 'branch', 'task'], value_vars=cols_of_interest)
...
for grp, df_subset in df_melted.groupby(['task', 'variable']):
    ...
    _ = model.fit(df_subset['video_duration_min'].to_numpy().reshape(-1, 1), df_subset['value'].to_numpy().reshape(-1, 1))
    plot = (
