...
out_onnx_model = 'onnx-models/single-mouse-pose/gait-model.onnx'
...
read_frame = imageio.imread(test_fname)
test_frame = np.expand_dims(read_frame.astype(np.float32), [0])
...
rendered_pose = render_pose_overlay(read_frame, pose, [])
imageio.imwrite('test-single-pose-output.png', rendered_pose)
