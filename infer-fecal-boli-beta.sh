#!/bin/bash
#
#SBATCH --job-name=infer-fecal-boli
#
#SBATCH --time=6:00:00
#SBATCH --ntasks=1
...
if [[ -n "${SLURM_JOB_ID}" ]]; then
	...
	if [[ -f "${1}" ]]; then
			...
			echo "Submitting ${NUM_VIDEOS} Videos in: ${1}"
			sbatch --export=FULL_VIDEO_FILE_LIST="${1}" --array=1-"$NUM_VIDEOS"%56 "${0}"
	else
			echo "ERROR: you need to provide a video file to process. Eg: ./infer-fecal-boli-beta.sh /full/path/movie_list.txt" >&2
			exit 1
	fi
fi
