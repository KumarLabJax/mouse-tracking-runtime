...
def predict_pose_topdown(input_iter, mask_file, model, render: str = None, batch_size: int = 1):
	...
	while not video_done:
		...
		if batch_count == 0:
			...
			pose_results.results_receiver_queue.put((num_frames_in_batch, default_pose), timeout=5)
			confidence_results.results_receiver_queue.put((num_frames_in_batch, default_conf), timeout=5)
			t4 = time.time()
			# compute skipped
		...
		pose_stacked = np.full([num_frames_in_batch, np.max(batch_frame_count), 12, 2], 0, np.int64)
		conf_stacked = np.full([num_frames_in_batch, np.max(batch_frame_count), 12], 0, np.float32)
		cur_idx = 0
		for cur_frame_idx, num_obs in enumerate(batch_frame_count):
			if num_obs == 0:
				continue
			pose_stacked[cur_frame_idx, :num_obs] = pose[cur_idx:(cur_idx + num_obs)]
			conf_stacked[cur_frame_idx, :num_obs] = confidence[cur_idx:(cur_idx + num_obs)]
			cur_idx += num_obs

		try:
			pose_results.results_receiver_queue.put((num_frames_in_batch, pose_stacked), timeout=5)
			confidence_results.results_receiver_queue.put((num_frames_in_batch, conf_stacked), timeout=5)
		except queue.Full:
			if not pose_results.is_healthy() or not confidence_results.is_healthy():
				print('Writer thread died unexpectedly.', file=sys.stderr)
				sys.exit(1)
			print(f'WARNING: Skipping inference on batch: {batch_num}, frames: {frame_idx - num_frames_in_batch}-{frame_idx - 1}')
			continue
		if render is not None:
			for idx in np.arange(num_frames_in_batch):
				...
				for cur_frame_idx in np.arange(pose_stacked.shape[1]):
					...
					rendered_pose = render_pose_overlay(rendered_pose, current_pose, np.argwhere(current_confidence == 0).flatten())
...
def infer_multi_pose_pytorch(args):
	...
	pose_results, confidence_results, performance_accumulator = predict_pose_topdown(frame_iter, args.out_file, model, args.out_video, args.batch_size)
