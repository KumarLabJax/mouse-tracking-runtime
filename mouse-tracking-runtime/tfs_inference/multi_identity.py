...
def infer_multi_identity_tfs(args):
    ...
    colour_results = prediction_saver(dtype=np.float32)
    ...
    for frame_idx, frame in enumerate(frame_iter):
        ...
        colour_frame = np.zeros([pose_data.shape[1], 1], dtype=np.float32) ##### Change
        valid_poses = np.arange(pose_data.shape[1])
        # Rotate and crop each pose instance
        for animal_idx in np.arange(pose_data.shape[1]):
            try:
                transformed_frame = crop_and_rotate_frame(frame, pose_data[frame_idx, animal_idx], [128, 128])
                input_frames[animal_idx] = transformed_frame[:, :, 0]
                #### Change
                
                centre_vals = [(62, 62), (62, 63), (62, 64), (62, 65), (63, 62), (63, 63), (63, 64), (63, 65), (64, 62), (64, 63), (64, 64), (64, 65), (65, 62), (65, 63), (65, 64), (65, 65)]
                central_pixels = np.array([input_frames[animal_idx, row, col] for row, col in centre_vals])
                colour_frame[animal_idx] = central_pixels.mean(axis=0)
        ...
        for animal_idx in valid_poses:
            prediction = model.signatures['serving_default'](tf.convert_to_tensor(input_frames[animal_idx].reshape([1, 128, 128, 1])))
        ...
        try:
            embedding_results.results_receiver_queue.put((1, np.expand_dims(prediction_matrix, (0))), timeout=5)
            colour_results.results_receiver_queue.put((1, np.expand_dims(colour_frame, (0))), timeout=5)
        except queue.Full:
            if not embedding_results.is_healthy() or colour_results.is_healthy(): 
    ...
    colour_results.results_receiver_queue.put((None,None))
    final_embedding_matrix = embedding_results.get_results()
    final_colour_array = colour_results.get_results()
    write_identity_data(args.out_file, final_embedding_matrix, final_colour_array, model_definition['model-name'], model_definition['model-checkpoint'])
