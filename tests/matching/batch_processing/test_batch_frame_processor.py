...
class TestBatchedFrameProcessorProcessFrameBatch:
    ...
    def test_process_frame_batch_basic(self):
        ...
        mock_video_obs._calculate_costs_vectorized = Mock(return_value=np.array([
            [1.0, 2.0],
            [3.0, 1.5]
        ]))
        
        # Mock existing frame dict
        frame_dict = {0: {0: 0, 1: 1}}  # Frame 0 maps detection 0->tracklet 0, detection 1->tracklet 1
        
        # Mock greedy matching
        with patch('mouse_tracking.matching.batch_processing.vectorized_greedy_matching') as mock_matching:
    ...
    def test_process_frame_batch_cost_calculation_calls(self):
        ...
        # Mock cost calculation
        mock_video_obs._calculate_costs_vectorized = Mock(return_value=np.array([[1.0]]))
    ...
    def test_process_frame_batch_empty_frames(self):
        ...
        # Mock cost calculation
        mock_video_obs._calculate_costs_vectorized = Mock(return_value=np.array([]).reshape(1, 0))
    ...
    def test_process_frame_batch_tracklet_id_continuity(self):
        ...
        # Mock cost calculation
        mock_video_obs._calculate_costs_vectorized = Mock(side_effect=[
...
class TestBatchedFrameProcessorIntegration:
    ...
    def test_batch_processing_with_different_parameters(self):
        ...
        with patch('mouse_tracking.matching.batch_processing.vectorized_greedy_matching') as mock_matching:
            ...
            result_no_rotate = processor._process_frame_batch(
                mock_video_obs, frame_dict, 1, 1, 2, 10.0, False
            )
            
            # Test with rotate_pose=True
            result_with_rotate = processor._process_frame_batch(
                mock_video_obs, frame_dict, 1, 1, 2, 10.0, True
            )
        
        # Check that cost calculation was called with correct rotate_pose parameter
        calls = mock_video_obs._calculate_costs_vectorized.call_args_list
        assert calls[0][0][2] == False  # First call with rotate_pose=False
        assert calls[1][0][2] == True   # Second call with rotate_pose=True
    ...
    def test_error_handling_in_batch_processing(self):
        ...
        mock_video_obs._calculate_costs_vectorized = Mock(side_effect=RuntimeError("Test error"))
