...
class TestComputeVectorizedEmbeddingDistances:
    ...
    def test_basic_embedding_distance(self, features_factory):
        ...
        features1 = features_factory(
            n_detections=1, 
            embed_configs=[embed_configs[0]],
            seed=42
        )
        features2 = features_factory(
            n_detections=1, 
            embed_configs=[embed_configs[1]],
            seed=42
        )
        ...
        expected = scipy.spatial.distance.cdist([embed1], [embed2], metric='cosine')[0, 0]
    ...
    def test_identical_embeddings(self, features_factory):
        ...
        features1 = features_factory(n_detections=1, embed_configs=embed_configs, seed=42)
        features2 = features_factory(n_detections=1, embed_configs=embed_configs, seed=42)
    ...
    def test_orthogonal_embeddings(self, features_factory):
        ...
        features1 = features_factory(n_detections=1, embed_configs=[{'has_embedding': False}])
        features2 = features_factory(n_detections=1, embed_configs=[{'has_embedding': False}])
    ...
    def test_matrix_computation(self, features_factory):
        ...
        features1 = features_factory(n_detections=2, embed_configs=embed_configs[:2], seed=42)
        features2 = features_factory(n_detections=3, embed_configs=embed_configs, seed=100)
        ...
        expected_01 = scipy.spatial.distance.cdist([features1.embeddings[0]], [features2.embeddings[1]], metric='cosine')[0, 0]
        expected_01 = np.clip(expected_01, 0, 1.0 - 1e-8)
        np.testing.assert_allclose(result[0, 1], expected_01, rtol=1e-10)
    
    def test_consistency_with_original_method(self, detection_factory, features_factory):
...
class TestComputeVectorizedEmbeddingDistancesEdgeCases:
    ...
    def test_empty_embeddings_both_sides(self, features_factory):
        ...
        embed_configs2 = [{'has_embedding': False}, {'has_embedding': False}, {'has_embedding': False}]
    ...
    def test_empty_embeddings_one_side(self, features_factory):
        ...
        embed_configs_valid = [{'has_embedding': True, 'dim': 64}, {'has_embedding': True, 'dim': 64}]
    ...
    def test_mixed_valid_invalid_embeddings(self, features_factory):
        ...
        features1 = features_factory(n_detections=2, embed_configs=[
            {'has_embedding': True, 'dim': 32, 'value': 0.5},  # Valid
            {'has_embedding': False}  # Invalid (will be zeros)
        ])
        features2 = features_factory(n_detections=2, embed_configs=[
            {'has_embedding': False},  # Invalid (will be zeros)  
    ...
    def test_single_detection_each_side(self, features_factory):
        ...
        features1 = features_factory(n_detections=1, embed_configs=[{'has_embedding': True, 'dim': 16}])
        features2 = features_factory(n_detections=1, embed_configs=[{'has_embedding': True, 'dim': 16}])
...
class TestComputeVectorizedEmbeddingDistancesProperties:
    ...
    def test_distance_symmetry(self, features_factory):
        ...
        features = features_factory(n_detections=3, embed_configs=[
    ...
    def test_random_embedding_consistency(self, features_factory):
        ...
        features1 = features_factory(n_detections=n1, embed_configs=[{'has_embedding': False}] * n1)
        features2 = features_factory(n_detections=n2, embed_configs=[{'has_embedding': False}] * n2)
        ...
        expected = scipy.spatial.distance.cdist(embeddings1, embeddings2, metric='cosine')
        expected = np.clip(expected, 0, 1.0 - 1e-8)
...
class TestComputeVectorizedEmbeddingDistancesPerformance:
    ...
    def test_large_matrix_computation(self, features_factory):
        ...
        features2 = features_factory(n_detections=n2, embed_configs=[
...
class TestComputeVectorizedEmbeddingDistancesIntegration:
    ...
    def test_match_original_distance_matrix(self, detection_factory, features_factory):
        ...
        for i in range(n):
            for j in range(n):
                original_matrix[i, j] = Detection.embed_distance(detections[i].embed, detections[j].embed)
        
        # Compute vectorized distance matrix
        features = features_factory(n_detections=n, embed_configs=[{'has_embedding': False}] * n)
        ...
        if np.any(valid_mask):
            np.testing.assert_allclose(
                original_matrix[valid_mask], 
                vectorized_matrix[valid_mask], 
                rtol=1e-15
            )
