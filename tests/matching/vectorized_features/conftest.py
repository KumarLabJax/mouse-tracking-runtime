...
def detection_factory(mock_detection, sample_pose_data, sample_embedding_data, sample_segmentation_data):
    ...
    def _create_detection(
        ...
        embed = sample_embedding_data(dim=embed_dim, value=embed_value, seed=seed) if has_embedding else None
        
        # Generate segmentation data
        seg_mat = sample_segmentation_data(shape=seg_shape, seed=seed) if has_segmentation else None
...
def features_factory(detection_factory):
    ...
    def _create_features(
        n_detections: int = 3,
        pose_configs: list = None,
        embed_configs: list = None,
        seg_configs: list = None,
        seed: int = 42,
    ):
