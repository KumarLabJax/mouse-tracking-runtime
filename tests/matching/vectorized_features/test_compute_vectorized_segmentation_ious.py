...
class TestComputeVectorizedSegmentationIous:
    ...
    def test_basic_segmentation_iou(self, features_factory):
        ...
        features1 = features_factory(
            n_detections=1, 
            seg_configs=[seg_configs[0]],
            seed=42
        )
        features2 = features_factory(
            n_detections=1, 
            seg_configs=[seg_configs[1]],
            seed=42
        )
        
        # Mock render_blob to return predictable segmentation images
        with patch('mouse_tracking.matching.vectorized_features.render_blob') as mock_render:
    ...
    def test_matrix_computation(self, features_factory):
        ...
        features1 = features_factory(n_detections=2, seg_configs=seg_configs[:2], seed=42)
    ...
    def test_consistency_with_original_method(self, features_factory):
        ...
        features1 = features_factory(n_detections=1, seg_configs=[{'has_segmentation': True}], seed=42)
        features2 = features_factory(n_detections=1, seg_configs=[{'has_segmentation': True}], seed=100)
...
class TestComputeVectorizedSegmentationIousEdgeCases:
    ...
    def test_missing_segmentations_both_sides(self, features_factory):
        ...
        seg_configs2 = [{'has_segmentation': False}, {'has_segmentation': False}, {'has_segmentation': False}]
    ...
    def test_mixed_valid_invalid_segmentations(self, features_factory):
        ...
        features2 = features_factory(n_detections=2, seg_configs=[
            {'has_segmentation': False},  # Invalid  
            {'has_segmentation': True}    # Valid
        ])
    ...
    def test_single_detection_each_side(self, features_factory):
        ...
        features1 = features_factory(n_detections=1, seg_configs=[{'has_segmentation': True}])
        features2 = features_factory(n_detections=1, seg_configs=[{'has_segmentation': True}])
    ...
    def test_special_case_one_has_seg_mat_other_none(self, features_factory):
        ...
        features1.detections[0]._seg_mat = np.array([[[1, 2], [3, 4]]])  # Has segmentation data
...
class TestComputeVectorizedSegmentationIousProperties:
    ...
    def test_iou_symmetry(self, features_factory):
        ...
        with patch('mouse_tracking.matching.vectorized_features.render_blob') as mock_render:
            ...
            mock_render.side_effect = seg_images + seg_images  # Called twice for symmetric computation
...
class TestComputeVectorizedSegmentationIousPerformance:
    ...
    def test_large_matrix_computation(self, features_factory):
        ...
        features1 = features_factory(n_detections=n1, seg_configs=[
            {'has_segmentation': True} for _ in range(n1)
        ], seed=42)
        features2 = features_factory(n_detections=n2, seg_configs=[
            {'has_segmentation': True} for _ in range(n2)
        ], seed=100)
...
class TestComputeVectorizedSegmentationIousIntegration:
    ...
    def test_match_original_iou_matrix(self, features_factory):
        ...
        with patch('mouse_tracking.matching.vectorized_features.render_blob') as mock_render:
            ...
            expected_iou = np.sum(np.logical_and(seg_image1, seg_image2)) / np.sum(np.logical_or(seg_image1, seg_image2))
            np.testing.assert_allclose(vectorized_matrix[0, 1], expected_iou, rtol=1e-15)
            np.testing.assert_allclose(vectorized_matrix[1, 0], expected_iou, rtol=1e-15)
    ...
    def test_caching_behavior(self, features_factory):
        ...
        features = features_factory(n_detections=2, seg_configs=[
            {'has_segmentation': True},
            {'has_segmentation': True}
        ])
