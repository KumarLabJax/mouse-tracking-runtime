...
class TestComputeVectorizedMatchCosts:
    ...
    def test_basic_match_cost_computation(self, features_factory):
        ...
        with patch.multiple(
            ...
            compute_vectorized_embedding_distances=MagicMock(return_value=np.array([[0.5]])),
            compute_vectorized_segmentation_ious=MagicMock(return_value=np.array([[0.3]])),
        ):
...
class TestComputeVectorizedMatchCostsParameters:
    ...
    def test_beta_parameter_validation(self, features_factory):
        ...
        # Valid beta
        result = compute_vectorized_match_costs(features1, features2, beta=(1.0, 1.0, 1.0))
        assert result.shape == (1, 1)
        ...
        with pytest.raises(AssertionError):
            compute_vectorized_match_costs(features1, features2, beta=(1.0, 1.0, 1.0, 1.0))
    ...
    def test_default_cost_parameter_handling(self, features_factory):
        ...
        features1 = features_factory(n_detections=1, seg_configs=[{'has_segmentation': False}], 
                                   embed_configs=[{'has_embedding': False}])
        features2 = features_factory(n_detections=1, seg_configs=[{'has_segmentation': False}], 
                                   embed_configs=[{'has_embedding': False}])
        ...
        # Tuple default_cost
        result2 = compute_vectorized_match_costs(features1, features2, default_cost=(0.1, 0.2, 0.3))
        ...
        with pytest.raises(AssertionError):
            compute_vectorized_match_costs(features1, features2, default_cost=(0.1, 0.2))
    ...
    def test_beta_weighting(self, features_factory):
        ...
        result_equal = compute_vectorized_match_costs(features1, features2, beta=(1.0, 1.0, 1.0))
        result_pose_only = compute_vectorized_match_costs(features1, features2, beta=(1.0, 0.0, 0.0))
        result_embed_only = compute_vectorized_match_costs(features1, features2, beta=(0.0, 1.0, 0.0))
        result_seg_only = compute_vectorized_match_costs(features1, features2, beta=(0.0, 0.0, 1.0))
    ...
    def test_pose_rotation_parameter(self, features_factory):
        ...
        result_no_rotation = compute_vectorized_match_costs(features1, features2, pose_rotation=False)
        result_with_rotation = compute_vectorized_match_costs(features1, features2, pose_rotation=True)
    ...
    def test_max_dist_parameter(self, features_factory):
        ...
        result_small = compute_vectorized_match_costs(features1, features2, max_dist=20.0)
        result_large = compute_vectorized_match_costs(features1, features2, max_dist=100.0)
...
class TestComputeVectorizedMatchCostsEdgeCases:
    ...
    def test_missing_data_handling(self, features_factory):
        ...
        features1 = features_factory(n_detections=2, seg_configs=[
        ...
        features2 = features_factory(n_detections=1, seg_configs=[
            {'has_segmentation': True}    # Has segmentation
        ], embed_configs=[
            {'has_embedding': True}       # Has embedding
        ])
    ...
    def test_no_detections(self, features_factory):
        ...
        pytest.skip("Empty detection arrays need special handling in vectorized functions")
    ...
    def test_extreme_parameter_values(self, features_factory):
        ...
        # Very small max_dist
        result_small = compute_vectorized_match_costs(features1, features2, max_dist=0.1)
        assert np.isfinite(result_small[0, 0])
        
        # Very large max_dist
        result_large = compute_vectorized_match_costs(features1, features2, max_dist=1000.0)
        assert np.isfinite(result_large[0, 0])
        
        # Very small beta weights
        result_small_beta = compute_vectorized_match_costs(features1, features2, beta=(0.01, 0.01, 0.01))
        assert np.isfinite(result_small_beta[0, 0])
        
        # Very large beta weights
        result_large_beta = compute_vectorized_match_costs(features1, features2, beta=(100.0, 100.0, 100.0))
...
class TestComputeVectorizedMatchCostsIntegration:
    ...
    def test_cost_computation_logic(self, features_factory):
        ...
        with patch.multiple(
            'mouse_tracking.matching.vectorized_features',
            compute_vectorized_pose_distances=MagicMock(return_value=np.array([[np.nan]])),  # Invalid pose
            compute_vectorized_embedding_distances=MagicMock(return_value=np.array([[0.8]])),  # Valid embedding
            compute_vectorized_segmentation_ious=MagicMock(return_value=np.array([[np.nan]])),  # Invalid segmentation
        ):
            result = compute_vectorized_match_costs(
                features1, features2,
                max_dist=40.0,
...
class TestComputeVectorizedMatchCostsProperties:
    ...
    def test_beta_scaling_properties(self, features_factory):
        ...
        result1 = compute_vectorized_match_costs(features1, features2, beta=(1.0, 1.0, 1.0))
        result2 = compute_vectorized_match_costs(features1, features2, beta=(2.0, 2.0, 2.0))
    ...
    def test_default_cost_effect(self, features_factory):
        ...
        result_low = compute_vectorized_match_costs(features1, features2, default_cost=0.1)
        result_high = compute_vectorized_match_costs(features1, features2, default_cost=0.9)
    ...
    def test_max_dist_effect(self, features_factory):
        ...
        result_small = compute_vectorized_match_costs(features1, features2, max_dist=10.0, beta=(1.0, 0.0, 0.0))
        result_large = compute_vectorized_match_costs(features1, features2, max_dist=100.0, beta=(1.0, 0.0, 0.0))
    ...
    def test_mathematical_consistency(self, features_factory):
        ...
        with patch.multiple(
            'mouse_tracking.matching.vectorized_features',
            compute_vectorized_pose_distances=MagicMock(return_value=np.array([[0.0]])),  # Perfect pose match
            compute_vectorized_embedding_distances=MagicMock(return_value=np.array([[0.0]])),  # Perfect embedding match
            compute_vectorized_segmentation_ious=MagicMock(return_value=np.array([[1.0]])),  # Perfect segmentation match
        ):
            result = compute_vectorized_match_costs(
                features1, features2,
                max_dist=40.0,
