...
class TestSinglePoseImplementation:
    ...
    def test_single_pose_input_validation(
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_file_existence_validation(
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_optional_parameters(self, mock_infer, out_video, batch_size):
        """
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_batch_size_validation(self, mock_infer, batch_size):
        """
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_default_values(self, mock_infer):
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_integration_flow(self, mock_infer):
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_video_input_processing(self, mock_infer):
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_frame_input_processing(self, mock_infer):
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_edge_case_paths(self, mock_infer, edge_case_path):
        """
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_batch_size_edge_cases(self, mock_infer, batch_size):
        """
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_gait_paper_model_specific(self, mock_infer):
        """Test gait-paper model specific functionality."""
        # Arrange
        cmd_args = [
            "single-pose",
            "--out-file",
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_minimal_configuration(self, mock_infer):
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_maximum_configuration(self, mock_infer):
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_comparison_with_multi_pose(self, mock_infer):
        """Test that single-pose can use same batch sizes as multi-pose."""
        # Arrange - Test that single-pose supports similar batch sizes to multi-pose
        cmd_args = [
            "single-pose",
            "--out-file",
            str(self.test_output_path),
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_simplified_output_options(self, mock_infer):
        """Test simplified output options compared to other commands."""
        # Arrange - single-pose only has out-video, no out-image
        cmd_args = [
            "single-pose",
            "--out-file",
            str(self.test_output_path),
        ...
        with patch("pathlib.Path.exists", return_value=True):
            ...
            args = mock_infer.call_args[0][0]
            assert args.out_video == str(self.test_video_output_path)
            # single-pose doesn't have out_image parameter
            assert not hasattr(args, "out_image")

    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_pytorch_runtime_consistency(self, mock_infer):
        """Test PyTorch runtime consistency with multi-pose."""
        # Arrange
        cmd_args = [
            "single-pose",
            "--out-file",
    ...
    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_gait_vs_multi_pose_topdown_models(self, mock_infer):
        """Test that single-pose uses gait-paper vs multi-pose topdown model."""
        # Arrange
        cmd_args = [
            "single-pose",
            "--out-file",
        ...
        with patch("pathlib.Path.exists", return_value=True):
            ...
            args = mock_infer.call_args[0][0]
            assert args.model == "gait-paper"
            # Different from multi-pose which uses "social-paper-topdown"

    @patch("mouse_tracking.cli.infer.infer_single_pose_pytorch")
    def test_single_pose_args_compatibility_object(self, mock_infer):
