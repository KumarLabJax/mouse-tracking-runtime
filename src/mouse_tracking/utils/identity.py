...
def get_rotation_mat(pose: np.ndarray, input_size: tuple[int], output_size: tuple[int]) -> np.ndarray:
	...
	"""
	masked_pose = np.ma.array(np.flip(pose, axis=-1), mask=np.repeat(np.all(pose == 0, axis=-1), 2).reshape(pose.shape))
	if np.all(masked_pose.mask[0:10]):
		raise InvalidIdentityException('Pose required at least 1 keypoint on the main torso to crop and rotate frame.')
	if np.all(masked_pose.mask[0:4]):
		raise InvalidIdentityException('Pose required at least 1 keypoint on the front to crop and rotate frame.')
	# Use all non-tail keypoints for center of crop
	center = ((np.max(masked_pose[0:10], axis=0) + np.min(masked_pose[0:10], axis=0)) / 2).filled()
	# Use the face keypoints for center direction
	center_face = ((np.max(masked_pose[0:4], axis=0) + np.min(masked_pose[0:4], axis=0)) / 2).filled()
	...
	translate_2 = np.array([[1, 0, output_size[0] / 2], [0, 1, output_size[1] / 2], [0, 0, 1]])
