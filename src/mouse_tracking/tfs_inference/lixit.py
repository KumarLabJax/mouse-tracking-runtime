...
def infer_lixit_model(args):
	...
	for frame_idx, frame in enumerate(frame_iter):
		...
		try:
			...
			lixit_results.results_receiver_queue.put((1, np.expand_dims(predicted_keypoints, axis=0)), timeout=5)
	...
	lixit_matrix = np.ma.array(lixit_matrix, mask=np.repeat(np.all(lixit_matrix == 0, axis=-1), 2).reshape(lixit_matrix.shape)).reshape([-1, 2])
	if np.all(lixit_matrix.mask):
		...
		if args.out_file is not None:
			write_static_object_data(args.out_file, filtered_keypoints, 'lixit', model_definition['model-name'], model_definition['model-checkpoint'])
