...
def infer_single_segmentation_tfs(args):
	...
	with tf.Session(graph=tf.Graph(), config=core_config) as session:
		model = tf.saved_model.loader.load(session, ['serve'], model_definition['tfs-model'])
		...
		for frame_idx, frame in enumerate(frame_iter):
			t1 = time.time()
			input_frame = np.reshape(cv2.resize(frame[:, :, 0], [480, 480]), [1, 480, 480, 1]).astype(np.float32)
			t2 = time.time()
			prediction = session.run([output_tensor], feed_dict={input_tensor: input_frame})
			t3 = time.time()
