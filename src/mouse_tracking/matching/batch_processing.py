...
class BatchedFrameProcessor:
	...
	def process_video_observations(self, video_observations: 'VideoObservations', 
								   max_cost: float = -np.log(1e-3), 
								   rotate_pose: bool = False) -> dict:
		...
		if n_frames <= 1:
			return {0: {i: i for i in range(len(observations[0]))}} if n_frames == 1 else {}
		...
		for batch_start in range(1, n_frames, self.batch_size):
			...
			batch_results = self._process_frame_batch(
				video_observations, frame_dict, cur_tracklet_id,
				batch_start, batch_end, max_cost, rotate_pose
			)
	...
	def _process_frame_batch(self, video_observations: 'VideoObservations',
							 frame_dict: dict, cur_tracklet_id: int,
							 batch_start: int, batch_end: int,
							 max_cost: float, rotate_pose: bool) -> dict:
		...
		return {
			'frame_dict': batch_frame_dict,
			'next_tracklet_id': cur_tracklet_id
		}
