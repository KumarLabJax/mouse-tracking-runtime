...
def get_contours(mask_img: np.ndarray, min_contour_area: float = 50.0) -> List[np.ndarray]:
	...
	"""
	if np.any(mask_img):
		contours, tree = cv2.findContours(mask_img.astype(np.uint8), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
...
def pad_contours(contours: List[np.ndarray], default_val: int = -1) -> np.ndarray:
	...
	padded_matrix = np.full([num_contours, max_contour_length, 2], default_val, dtype=np.int32)
