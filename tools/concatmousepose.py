"""
Convert CVAT annotations to an HDF annotation file
"""
...
#   --hdf5-in data/hdf5mouse/point_tracker_withSeg+HE_Validfix.h5 \
#   --cvat-anno data/hdf5mouse/CCF1_Annotations.xml data/hdf5mouse/CCF1_Validation.txt data/hdf5mouse/CCF1_TrainingFrames \
#   --cvat-anno data/hdf5mouse/22_KOMP2_Annotations.xml data/hdf5mouse/KOMP2_Validation.txt data/hdf5mouse/KOMP2_TrainingFrames/Ref \
...
def main():
    ...
    with h5py.File(args.hdf5_out, 'w') as hdf5_out:
        ...
        for hdf5_in_filename in args.hdf5_in:
            with h5py.File(hdf5_in_filename, 'r') as hdf5_in:
                for grp_name1, grp1 in hdf5_in.items():
                    # print(grp_name1, isinstance(grp1, h5py.Group), isinstance(grp1, h5py.Dataset))
                    if isinstance(grp1, h5py.Group):
                        for grp_name2, grp2 in grp1.items():
                            # print('\t', grp_name2, isinstance(grp2, h5py.Group), isinstance(grp2, h5py.Dataset))
                            if isinstance(grp2, h5py.Group):
                                for dataset_name, dataset in grp2.items():
                                    # print('\t\t', dataset_name, isinstance(dataset, h5py.Group), isinstance(dataset, h5py.Dataset))
                                    if isinstance(dataset, h5py.Dataset):
                                        hdf5_out[grp_name1 + '/' + grp_name2 + '/' + dataset_name] = dataset[()]
